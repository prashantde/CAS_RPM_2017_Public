...
download.data <- function(data_dir) {
  ...
  if (!file.exists(paste0(data_dir,'input.txt'))) {
    download.file(url='http://cs.stanford.edu/people/karpathy/char-rnn/shakespear.txt', destfile=paste(data_dir,'input.txt'), method='curl')
  }
}
...
#download.file(url='http://cs.stanford.edu/people/karpathy/char-rnn/shakespear.txt', destfile=paste('C:/Desktop','input.txt',sep=""), method='wget')
ret <- make.data("C:/Users/pde/Desktop/Text/input.txt", seq.len=seq.len)
